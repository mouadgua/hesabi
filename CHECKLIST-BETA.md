...
- [x] ~~**A1 — Sentry est un stub non fonctionnel**~~ — **✅ FAIT le 2026-08-10** (`feature/checklist-env-validation`)
  ...
  **Complété le 2026-08-13** : le DSN quitte les fichiers de config pour le `.env` (`SENTRY_DSN` + `NEXT_PUBLIC_SENTRY_DSN`), afin d'être configurable par déploiement. Les configs n'initialisent plus Sentry en son absence, pour ne pas émettre d'avertissement au démarrage pour une remontée qui n'irait nulle part.
  **Trou comblé au passage** : `instrumentation-client.js` n'appelait **jamais** `Sentry.init()` — les erreurs du navigateur (rendu React, promesses rejetées) n'étaient donc pas remontées, alors que ce sont celles que voit l'utilisateur. Vérifié : SDK chargé côté navigateur, erreur client transmise via le tunnel `/monitoring`, et **0 violation CSP** sur les pages authentifiées sous politique stricte.

...
export default function ExportModal({ selectedDocs, trigger }) {
  ...
  useEffect(() => {
    setColumns(prev => prev.map(c => ({
      ...c,
      label: FIELD_LABELS[c.key] ? colLabel(c.key, lang) : c.label,
    })))
  }, [lang])
  ...
  async function handleSaveTemplate() {
    ...
    setSaving(true)
    try {
      const config = { preset, lang, columns: columns.map((c, i) => ({ ...c, order: i })), groupBy, subtotals }
  ...
  function addCalcColumn() {
    ...
    setCalcLabel(''); setCalcFormula(''); setShowCalcForm(false)
  }

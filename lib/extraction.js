  ...
  for (const model of GEMINI_TEXT_MODELS) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        logger.warn('Modèle Gemini texte en échec', { raison: lastErr.message })
        continue
      }
      ...
      logger.info('Structuration Gemini réussie', { model, chars: text.length })
      return { content: text, model }
    } catch (err) {
      lastErr = err
      logger.warn('Exception sur modèle Gemini texte', { model, raison: err.message })
    } finally {
      clearTimeout(timer)
    }
  }
...
export async function extractDocument(base64, mimeType, prompt, {
  ...
  if (extractionMethod === 'azure') {
    logger.info('Méthode d\'extraction retenue', { methode: 'azure', etapes: 'Azure DI → extractEssential → Gemini texte' })
  ...
  if (extractionMethod === 'hybrid_azure') {
    logger.info('Méthode d\'extraction retenue', { methode: 'hybrid_azure', etapes: 'Azure DI → texte brut → Gemini texte' })

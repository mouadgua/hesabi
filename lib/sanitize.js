...
/**
 * Defense in depth against a client that bypasses the browser-side checks
 * (lib/clientValidation.js): reject empty files and truncated PDFs before
 * they can enter the extraction queue. Heuristic only for PDFs (checks for
 * the trailing %%EOF marker), not a full parse.
 * Returns { valid, error }
 */
export function validateFileIntegrity(mimeType, bytes) {
  if (!bytes || bytes.length === 0) {
    return { valid: false, error: 'Fichier vide.' }
  }
  if (mimeType === 'application/pdf') {
    const tailSize = Math.min(1024, bytes.length)
    const tail = bytes.slice(bytes.length - tailSize).toString('latin1')
    if (!tail.includes('%%EOF')) {
      return { valid: false, error: 'PDF tronqué ou illisible.' }
    }
  }
  return { valid: true }
}
